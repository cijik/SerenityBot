...
public class CharacterSheetService {
    ...
    public Mono<Message> getAllCharacters(ChatInputInteractionEvent event) {
        ...
        List<CharacterSheet> characters = characterSheetRepository.findAllByOwnerId(event.getInteraction().getUser().getId().asString()).collectList().block();
    ...
    public Mono<Message> updateCharacter(ChatInputInteractionEvent event) {
        ...
        CharacterSheet characterSheet = characterSheetRepository.findByName(WordUtils.capitalize(characterName.toLowerCase(Locale.ROOT))).block();
        if (characterSheet == null) {
            return createMissingCharacterFollowup(event, characterName);
        } else {
    ...
    public CharacterSheet getCharacterSheet(String characterName, String ownerId) {
        ...
        return characterSheetRepository.findByNameAndOwnerId(WordUtils.capitalize(characterName.toLowerCase(Locale.ROOT)), ownerId).block();
    }
